...
Clazz.load (["J.shape.AtomShape", "JU.P3", "$.V3"], "J.shapespecial.Polyhedra", ["java.lang.Boolean", "$.Float", "JU.AU", "$.BS", "$.Measure", "$.P3i", "$.SB", "J.c.PAL", "J.shapespecial.Polyhedron", "JU.C", "$.Logger", "$.Normix"], function () {
...
this.normixesT =  Clazz.newShortArray (250, 0);
this.planesT =  new Array (250);
...
return (bondCount < 3 || this.nVertices > 0 && !this.bsVertexCount.get (bondCount) ? null : this.validatePolyhedronNew (atom, bondCount, this.otherAtoms));
}, "~N");
...
return (otherAtomCount < 3 || this.nVertices > 0 && !this.bsVertexCount.get (otherAtomCount) ? null : this.validatePolyhedronNew (atom, otherAtomCount, this.otherAtoms));
...
var nother1 = ptCenter - 1;
var nother2 = ptCenter - 2;
var factor = (!Float.isNaN (this.distanceFactor) ? this.distanceFactor : nother1 <= 6 ? 1.85 : 1.5);
...
var faceCatalog = "";
var facetCatalog = "";
for (var i = 0; i < nother2; i++) for (var j = i + 1; j < nother1; j++) for (var k = j + 1; k < ptCenter; k++) if (this.isPlanar (points[i], points[j], points[k], points[ptCenter])) faceCatalog += this.faceId (i, j, k);



for (var j = 0; j < nother1; j++) for (var k = j + 1; k < ptCenter; k++) {
...
var isWindingOK = (isFlat ? JU.Measure.getNormalFromCenter (rpt, points[i], points[j], points[k], false, normal) : JU.Measure.getNormalFromCenter (points[ptCenter], points[i], points[j], points[k], true, normal));
...
facetCatalog += facet;
p[planeCount] = JU.P3i.new3 (isWindingOK ? i : j, isWindingOK ? j : i, nRef);
JU.Measure.getNormalFromCenter (points[k], points[i], points[j], ptRef, false, normal);
n[planeCount++] = (isFlat ? JU.Normix.get2SidedNormix (normal, bsT) : JU.Normix.getNormixV (normal, bsT));
}facet = this.faceId (i, k, -1);
if (collapsed || isFlat && facetCatalog.indexOf (facet) < 0) {
facetCatalog += facet;
p[planeCount] = JU.P3i.new3 (isWindingOK ? i : k, nRef, isWindingOK ? k : i);
JU.Measure.getNormalFromCenter (points[j], points[i], ptRef, points[k], false, normal);
n[planeCount++] = (isFlat ? JU.Normix.get2SidedNormix (normal, bsT) : JU.Normix.getNormixV (normal, bsT));
}facet = this.faceId (j, k, -1);
if (collapsed || isFlat && facetCatalog.indexOf (facet) < 0) {
facetCatalog += facet;
p[planeCount] = JU.P3i.new3 (nRef, isWindingOK ? j : k, isWindingOK ? k : j);
...
p[planeCount] = JU.P3i.new3 (isWindingOK ? i : j, isWindingOK ? j : i, k);
n[planeCount++] = JU.Normix.getNormixV (normal, bsT);
}}}
}

if (JU.Logger.debugging) JU.Logger.info ("planeCount=" + planeCount + " nPoints=" + nPoints);
return  new J.shapespecial.Polyhedron (centralAtom, ptCenter, nPoints, planeCount, otherAtoms, n, p, collapsed, offset, factor);
}, "JM.Atom,~N,~A");
...
"DEFAULT_MANY_VERTEX_DISTANCE_FACTOR", 1.5,
