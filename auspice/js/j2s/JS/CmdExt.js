...
Clazz.load (["JS.ScriptExt"], "JS.CmdExt", ["java.lang.Boolean", "$.Float", "$.Long", "java.util.Hashtable", "JU.AU", "$.BS", "$.Base64", "$.Lst", "$.M3", "$.M4", "$.Measure", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.STER", "$.VDW", "J.i18n.GT", "JM.Atom", "$.AtomCollection", "$.BondSet", "$.LabelToken", "JS.SV", "$.ScriptCompiler", "$.ScriptError", "$.ScriptEval", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.ScriptParam", "$.T", "JU.BSUtil", "$.BoxInfo", "$.C", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.Parser", "$.Point3fi", "JV.FileManager", "$.StateManager", "$.Viewer"], function () {
...
case 2:
var a = pts[1];
pts =  Clazz.newArray (-1, [pts[0], JU.P3.newP (pts[0]),  new JU.P3 (),  new JU.P3 (),  new JU.P3 ()]);
pts[1].x = a.x;
pts[2].y = a.y;
pts[3].z = a.z;
break;
...
}if (!this.chk && !this.vwr.getBoolean (603979888)) eval.setBooleanProperty ("navigationMode", true);
for (var i = 1; i < this.slen; i++) {
