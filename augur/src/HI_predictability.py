...
if mut_models:
    ...
    for fi,freq_thres in enumerate([0.25, 0.5, 0.75, 0.95]):
        ...
        ax3.errorbar(np.arange(len(frac_success))+0.5+0.03*fi, frac_success,stddev_success, label = r"$\mathrm{freq_{max}}>"+str(freq_thres)+"$", lw=2)
    ...
    plt.legend(loc=8, fontsize=fs, ncol=2)
    plt.ylim([0,1])
    ...
    plt.ylabel(r'fraction of $\mathrm{cHI}$ due $d_i<X$', fontsize = fs)
    plt.xlabel(r'contribution to HI titer $X$', fontsize = fs)
...
cb.set_label(r'recent antigenic evolution $[\log_2]$', fontsize=fs)
...
ax.plot(best_LBI[cutoff][:,0],best_LBI[cutoff][:,-3], label=r'LBI',lw=2)
ax.plot(best_HI[0.05][:,0],best_HI[0.05][:,-3], label=r'$\mathrm{cHI\; freq.}>0.05$',lw=2)
ax.plot(best_HI[0.01][:,0],best_HI[0.01][:,-3], label=r'$\mathrm{cHI\; freq.}>0.01$',lw=2)
...
ax.set_xlabel(r'antigenic advance $\mathrm{cHI}-\langle \mathrm{cHI}\rangle_{year}$', fontsize=fs)
ax.tick_params(labelsize=fs)
...
plt.ylabel(r'successful $\mathrm{cHI}-\langle \mathrm{cHI}\rangle_{year}$', fontsize=fs)
...
for li,lbi_cutoff in enumerate([0.2, 0.1]):
    ...
    ax.set_ylabel(r'$\mathrm{cHI}-\langle \mathrm{cHI}\rangle_{year}$', fontsize=fs)
    ax.set_xlabel(r'$LBI/\max(LBI)$', fontsize=fs)
    ax.tick_params(labelsize=fs)
