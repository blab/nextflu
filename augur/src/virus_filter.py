...
from H3N2_outgroup_and_vaccine import outgroup, vaccine_strains, min_length
...
class virus_filter(object):
	...
	def filter_generic(self, min_length=None, date_spec = 'full', prepend_strains = None):
		...
		self.filter_date(min_length, date_spec)
	...
	def subsample(self, years_back, viruses_per_month, prioritize = None):
		...
		priority_viruses = viruses_by_date_region([v for label,v in self.strain_lookup.iteritems() if v in prioritize]) 
		other_viruses = viruses_by_date_region([v for label,v in self.strain_lookup.iteritems() if v not in prioritize]) 
	...
	def select_viruses(priority_viruses,other_viruses, y, m, viruses_per_month, regions):
		'''
...
class flu_filter(virus_filter):
	...
	def __init__(self,fasta_fname):
		...
		self.viruses = self.parse_gisaid(fasta_fname)
