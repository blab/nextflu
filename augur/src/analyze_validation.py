...
for flu in ['H3N2', 'H1N1pdm', 'Vic', 'Yam']:
    for minaa in [0,1,'epi']:
        for training in ['measurements', 'virus']:
            fname = 'validation/'+'_'.join([flu, training, 'minaa', str(minaa)])+'.pkl'
            try:
                with open(fname) as infile:
                    params,grid, acc = cPickle.load(infile)
            except:
                ...
                for hi, lam_HI in enumerate(grid):
                    for pi, lam_pot in enumerate(grid):
                        for ai, lam_avi in enumerate(grid):
                            val_data.append([flu, str(minaa), training, lam_HI, lam_pot, lam_avi] +list(acc[hi,pi,ai]))
        ...
        for hi, lam_HI in enumerate(grid):
            for pi, lam_pot in enumerate(grid):
                try:
                    ...
                    ax =axs[0][hi]
                    ax.plot(grid, val_data.loc[ind,:]['abs'], c=cols[pi], ls = ls[mi], label='aa='+str(minaa)+', pot='+str(lam_pot))
                    if hi==len(grid)-1:
