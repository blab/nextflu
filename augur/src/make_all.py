...
if __name__=="__main__":
	...
	if 'H3N2' in run_pipeline:
		call = map(str, [params.bin, 'src/H3N2_process.py', '-v', 50, '-y', 3, '--skip', 'genotype_frequencies', '-r', 1.0, '--prefix', 'data/H3N2_'])
		print call
		subprocess.call(call)
	if 'H1N1' in run_pipeline:
		call = map(str, [params.bin, 'src/H1N1historical_process.py', '-v', 20, '--interval', 1990, 2010, '--skip', 'genotype_frequencies', '-r', 1.0, '--prefix', 'data/H1N1_'])
		print call
		subprocess.call(call)
	if 'H1N1pdm' in run_pipeline:
		call = map(str, [params.bin, 'src/H1N1pdm_process.py', '-v', 30, '-y', 6, '--skip', 'genotype_frequencies', '-r', 1.0, '--prefix', 'data/H1N1pdm_'])
		print call
		subprocess.call(call)
	if 'Vic' in run_pipeline:
		call = map(str, [params.bin, 'src/Vic_process.py', '-v', 30, '-y', 6, '--skip', 'genotype_frequencies', '-r', 1.0, '--prefix', 'data/Vic_'])
		print call
		subprocess.call(call)
	if 'Yam' in run_pipeline:
		call = map(str, [params.bin, 'src/Yam_process.py', '-v', 30, '-y', 6, '--skip', 'genotype_frequencies', '-r', 1.0, '--prefix', 'data/Yam_'])
		print call
		subprocess.call(call)
